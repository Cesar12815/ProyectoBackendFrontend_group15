...
export default function Texto({ title, date, description, isLast }: Props) {
  ...
  return (
    <article className="flex gap-5 mt-5" ref={achievementProperties}>
      <div className="flex items-center justify-center h-1 w-1 bg-blue-400 p-2 rounded-full relative ml-5">
        <span
          style={{
            height: `${spanHeight + 10}px`,
            bottom: `-${spanHeight + 10}px`,
          }}
          className={`${isLast ? "hidden" : ""} border-r border-blue-300 w-[1px] absolute`}
        ></span>
      </div>
